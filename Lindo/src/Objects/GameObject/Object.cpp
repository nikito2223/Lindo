...
void Object::calculateMeshBoundingBox() {
    if (!mesh) return;
    ...
    // Также вычисляем bounding sphere
...
void Object::calculateModelBoundingBox() {
    if (!model) return;

    // Пока просто создаем простой bbox для модели
    // Можно расширить, чтобы вычислять по всем мешам модели

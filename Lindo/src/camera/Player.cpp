...
void Player::setCrouching(bool crouch) {
    // Приводим коллайдер к типу капсулы
    auto capsuleCollider = std::dynamic_pointer_cast<CapsuleCollider>(collider);
    if (!capsuleCollider) return; // не капсула – выходим
    ...
    if (crouch) {
        ...
        // TODO: добавить raycast для проверки места над головой

        capsuleCollider->setHeight(newHeight);
        transform.position.y += heightDiff * 0.5f; // поднимаем центр

        movementSpeed = normalSpeed;  // возвращаем обычную скорость
        bCrouching = false;
    }
}

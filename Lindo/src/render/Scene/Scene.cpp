...
FirstPersonCamera* getActiveCamera() { return fpsCamera; }
...
void initScene() {
    ...
    playerObj->getCamera().setHeightOffset(0.88f);
    /*fpsCamera->setFront(glm::normalize(target - fpsCamera->getPosition()));*/

    // Создаём меш капсулы (упрощённый вариант - используем готовый цилиндр со сферами на концах)
    // В реальном проекте лучше загрузить модель или сгенерировать более качественный меш.
    // Здесь для демонстрации мы создадим объект без меша, но с коллайдером и визуализацией через отладку.
...
void renderScene(Shader& shader, float deltaTime, Shader* debugShader) {
    ...
    shader.setVec3("viewPos", fpsCamera->getPosition());
    ...
    if (spotLight) {
        spotLight->transform.position = fpsCamera->getPosition();
        spotLight->direction = fpsCamera->getFront();
...
void cleanupScene() {
    ...
    // Игрок удаляется отдельно (или можно тоже добавить в список)
    if (player) {
        delete player;
        player = nullptr;
    }

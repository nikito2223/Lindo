...
namespace Lindo {
    namespace Graphics {
        ...
        void ShadowManager::renderShadows(Lindo::World::Scene* scene,
            ...
            if (spotLight && spotLight->enabled && spotLight->castShadows && m_spotCaster) {
                ...
                if (spotLight->gameObject) {
                    lightDirection = glm::normalize(glm::mat3(spotLight->gameObject->getWorldMatrix()) * spotLight->direction);
                }
        ...
        void ShadowManager::bindShadowTextures(Shader& forwardShader, const Lindo::World::Scene* scene) const {
            ...
            // Это защищает от undefined behavior на некоторых драйверах, когда
            // sampler2DArray-юниформа объявлена в шейдере, но её текстурный
            // юнит вообще ничего не содержит — часть драйверов в этом случае

...
namespace Lindo {
    namespace Graphics {
        ...
        static Lindo::Core::Memory::ArrayPtr<float> parseRGBEStream(std::istream& stream, int& width, int& height) {
            std::string line;
            bool headerOk = false;
            ...
            width = w;
            height = h;
            auto pixels = Lindo::Core::Memory::Get().makeArray<float>(
        ...
        std::unique_ptr<Skybox> Skybox::CreateFromHDRData(const std::vector<char>& data, unsigned int resolution) {
        ...
        unsigned int Skybox::loadHDRTexture(const std::string& path) {
            ...
            auto data = parseRGBEStream(file, width, height);
            if (!data) return 0;

            unsigned int hdrTexture = 0;
            glGenTextures(1, &hdrTexture);
            glBindTexture(GL_TEXTURE_2D, hdrTexture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, width, height, 0, GL_RGB, GL_FLOAT, data.get());
        ...
        unsigned int Skybox::loadHDRTextureFromData(const std::vector<char>& data) {
            ...
            int width = 0, height = 0;
            auto pixels = parseRGBEStream(stream, width, height);
            if (!pixels) return 0;

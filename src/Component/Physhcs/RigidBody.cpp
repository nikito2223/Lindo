    ...
    namespace Components {
        namespace Physics {
            ...
            void RigidBody::applyForceAtPoint(const glm::vec3& force, const glm::vec3& worldPoint) {
                ...
                forceAccumulator += force;
                if (owner) {
                    glm::vec3 r = worldPoint - owner->transform.position;
            ...
            void RigidBody::OnUpdate(float deltaTime, const glm::vec3& gravity) {}
            ...
            glm::mat3 RigidBody::GetInertiaTensor() const {
                glm::vec3 half(0.5f);
                if (owner) {
                    half = glm::vec3(0.5f) * owner->transform.scale;
                }
                float m = mass;
            ...
            void RigidBody::integrate(float deltaTime, const glm::vec3& gravity) {
                if (!owner || invMass == 0.0f || isKinematic || isSleeping) return;
                ...
                if (torqueAccumulator != glm::vec3(0.0f)) {
                    glm::mat3 invInertia = glm::inverse(GetInertiaTensor());
                    angularVelocity += invInertia * torqueAccumulator * deltaTime;
                }
                ...
                if (glm::length(angularVelocity) > 1e-6f) {
                    glm::vec3 eulerDeg = owner->transform.rotation;
                    glm::vec3 deltaDeg = glm::degrees(angularVelocity) * deltaTime;
                    owner->transform.rotation = eulerDeg + deltaDeg;
                }
            ...
            void RigidBody::ApplyDamping(float deltaTime) {
                float linearF = std::max(0.0f, 1.0f - linearDamping * deltaTime * 60.0f);
                float angularF = std::max(0.0f, 1.0f - angularDamping * deltaTime * 60.0f);
                velocity *= linearF;

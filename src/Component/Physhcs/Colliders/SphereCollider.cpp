...
namespace Lindo {
    namespace Components {
        namespace Physics {
            ...
            bool SphereCollider::CheckCollision(const MeshCollider* other, CollisionInfo& outInfo) const {
                if (!other) return false;
                // Здесь логика проверки столкновения Box с MeshCollider 
                // (либо временно: return other->CheckCollision(this, outInfo); / заглушка)
                return false;
            }
            ...
            void SphereCollider::OnDrawGizmos() {
                ...
                // 2. Расчет радиуса с учетом максимального масштаба объекта
                float maxScale = glm::max(glm::max(glm::length(glm::vec3(world[0])),
                    glm::length(glm::vec3(world[1]))),
                    glm::length(glm::vec3(world[2])));
                float worldRadius = GetRadius() * maxScale;

                // 3. Матрица трансформации и быстрая GPU-отрисовка сферы
                glm::mat4 transform = glm::translate(glm::mat4(1.0f), pos);
                transform = glm::scale(transform, glm::vec3(worldRadius));

                debugDraw.DrawWireSphereFast(transform, glm::vec3(0.0f, 1.0f, 1.0f));
            }
